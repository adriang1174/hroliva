...
//Connection Settings @0-CE82F807
$CCConnectionSettings = array (
    "Connection1" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "dbpropiedades",
        "Host" => "localhost",
        "Port" => "",
        "User" => "root",
        "Password" => "",
...
//Initialize Common Variables @0-B7C86CC3
...
$CCSLocales->AddLocale("es-AR", Array("es", "AR", array(1, 0, ""), 2, ",", ".", array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"), array("Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"), array("Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"), array("Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"), array("D", "L", "M", "M", "J", "V", "S"), array("dd", "/", "mm", "/", "yyyy"), array("dddd", ", ", "dd", " de ", "mmmm", " de ", "yyyy"), array("hh", ":", "nn", " ", "tt"), array("hh", ":", "nn", ":", "ss", " ", "tt"), "a.m.", "p.m.", 0, true, "", "windows-1252", "CP1252"));
...
//CCLoginUser @0-F6838D5D
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT idlogin, idPerfil, password FROM usuarios WHERE login=" . $db->ToSQL($login, ccsText) . " AND password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("idlogin"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("idPerfil"));
    }
    return $Result;
}
//End CCLoginUser
